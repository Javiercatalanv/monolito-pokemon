...
def get_presets():
    ...
    for pr in PRESET_TEAMS:
        p_list = [
            pokemon_repo.get_by_id(pid) for pid in pr["pokemon_ids"] if pokemon_repo.get_by_id(pid)
        ]
        enriched_presets.append(
            {"name": pr["name"], "description": pr["description"], "pokemon": p_list}
        )
    return enriched_presets


@router.post(
    "/analyze", response_model=OpponentAnalysis, summary="Analizar debilidades de un equipo rival"
)
def analyze_opponent_team(request: CounterRequest):
    ...
    """
    opponent_pokemon = [
        pokemon_repo.get_by_id(pid)
        for pid in request.opponent_pokemon_ids
        if pokemon_repo.get_by_id(pid)
    ]
    if not opponent_pokemon:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No se encontraron Pokémon válidos con esos IDs",
        )
...
def generate_counter_team(request: CounterRequest):
    """
    Ejecuta el algoritmo multi-estrategia (balanced, offensive, defensive)
    para recomendar los 6 mejores counter Pokémon.
    """
    opponent_pokemon = [
        pokemon_repo.get_by_id(pid)
        for pid in request.opponent_pokemon_ids
